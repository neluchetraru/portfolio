...
function LiveBackground() {
  ...
  return (
    <canvas
      ...
      style={{
        maskImage:
          "linear-gradient(to bottom, black, rgba(0,0,0,0.55) 60%, rgba(0,0,0,0.35))",
      }}
    />
  );
}
        ...
        >
          ...
          <span
            className={`rounded-full transition-all duration-300 ${
              active === id
                ? "h-2 w-6 bg-accent"
                : "size-2 bg-stone-700 group-hover/rail:bg-stone-500"
            }`}
          />
        </a>
      ))}
...
function Contributions() {
  ...
  return (
    <section
      id="activity"
      className="reveal scroll-mt-16 border-t border-stone-800 py-16"
    >
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        >
          {days ? `${total.toLocaleString()} contributions — ` : ""}@
          {LINKS.githubUser}
        </a>
      </div>
      {days ? (
        ...
        <p className="mt-6 font-mono text-xs text-faint">
          Pulling this from GitHub. If it never shows up, it is all{" "}
...
/* stack — brand marks for the tools I work in */
function Stack() {
  return (
    <section
      id="stack"
    ...
    >
      ...
      <ul className="mt-10 grid grid-cols-4 gap-x-3 gap-y-8 sm:grid-cols-6 sm:gap-x-4 lg:grid-cols-8">
        {STACK.map((icon) => (
          <li
            key={icon.name}
          ...
          >
            <svg
              viewBox="0 0 24 24"
              aria-hidden
              className="size-9 fill-stone-400 transition-colors duration-200 group-hover/icon:fill-[var(--brand)]"
              style={{ "--brand": icon.color } as React.CSSProperties}
            >
              <path d={icon.path} />
            </svg>
            <span className="text-center font-mono text-[11px] text-stone-500 transition-colors duration-200 group-hover/icon:text-ink">
              {icon.name}
            </span>
          </li>
        ))}
...
function ProjectRow({ project, i }: { project: Project; i: number }) {
  return (
    <article
      id={`p-${project.index}`}
      className="reveal group relative grid scroll-mt-16 gap-5 border-t border-stone-800 py-10 md:grid-cols-[4rem_minmax(0,1fr)_minmax(0,24rem)] md:items-start md:gap-10 md:py-14"
      style={{ "--pa": project.accent } as React.CSSProperties}
    >
      <span
        className="hidden font-display text-4xl font-extralight md:block"
        style={{ color: "var(--pa)" }}
      >
        {project.index}
      </span>
      ...
      <div>
        <div className="flex items-baseline gap-3">
          <span
            className="font-display text-2xl font-extralight md:hidden"
            style={{ color: "var(--pa)" }}
          >
            {project.index}
          </span>
        ...
        <p className="mt-1 font-display text-lg italic text-stone-400">
          {project.tagline}
        </p>
        ...
        <img
          ...
          style={{
            boxShadow:
              "0 0 32px -8px color-mix(in oklch, var(--pa) 40%, transparent)",
          }}
        />
    ...
    <div className="relative">
      ...
      <div className="relative z-[1] mx-auto max-w-6xl px-6 pb-24">
        ...
        <section id="top" className="scroll-mt-24 py-20 md:py-28">
          ...
          >
            Full-stack software engineer based in Copenhagen, Denmark, with a
            degree in Computer Science & Engineering from Technical University
            of Denmark. I specialize in building web applications and have a
            strong interest in open-source software, cloud computing, and modern
            web technologies.
          </p>
          <div
        ...
        <section
          id="contact"
          className="reveal scroll-mt-16 border-t border-stone-800 py-28"
        >
          <h2 className="font-display text-4xl font-semibold tracking-tight md:text-5xl">
            Get in touch
            <span className="italic text-stone-400">building</span>
          </h2>
